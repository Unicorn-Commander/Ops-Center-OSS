...
4. Homepage URL: `https://unicorncommander.ai`
5. Authorization callback URL: `https://unicorncommander.ai/auth/callback`
...
6. Redirect URI: `https://unicorncommander.ai/auth/callback`
...
5. Set up webhook endpoint:
   - URL: `https://unicorncommander.ai/api/v1/stripe/webhook`
...
1. **Visit login page**: https://unicorncommander.ai/login.html
...
5. **Check user in database**:
   ```bash
   sqlite3 /home/deploy/Production/UC-1-Pro/volumes/ops_center.db
   SELECT * FROM users;
   .exit
   ```
...
1. **Visit pricing page**: https://unicorncommander.ai/pricing
...
1. **Make API request**:
   ```bash
   curl -X POST https://unicorncommander.ai/api/v1/inference \
...
2. **Check usage stats**:
   ```bash
   curl https://unicorncommander.ai/api/v1/user/usage \
     -H "Cookie: session_token=YOUR_SESSION_TOKEN"
   ```
3. **Verify in Lago**:
