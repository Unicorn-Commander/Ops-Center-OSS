...
headers = {
    "Authorization": "Bearer sk-loopnet-service-key-REDACTED",
...
SERVICE_KEYS = {
    ...
    'sk-loopnet-service-key-REDACTED': 'loopnet-service',  # ADD THIS
    'sk-centerdeep-service-key-REDACTED': 'centerdeep-service'  # ADD THIS
}
```
...
if __name__ == "__main__":
    # Initialize client
    client = OpsLLMClient("sk-loopnet-service-key-REDACTED")
...
// Usage Example
(async () => {
  const client = new OpsLLMClient('sk-loopnet-service-key-REDACTED');
...
curl -X POST http://ops-center-centerdeep:8084/api/v1/llm/chat/completions \
  -H "Authorization: Bearer sk-loopnet-service-key-REDACTED" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4o-mini",
    "messages": [
...
- [ ] Configure environment variable:
  ```bash
  OPS_LLM_API=http://ops-center-centerdeep:8084/api/v1/llm
  OPS_LLM_SERVICE_KEY=sk-loopnet-service-key-REDACTED
  ```
...
- [ ] Configure environment variable:
  ```bash
  OPS_LLM_API=http://ops-center-centerdeep:8084/api/v1/llm
  OPS_LLM_SERVICE_KEY=sk-centerdeep-service-key-REDACTED
  ```
- [ ] Create organization in Ops-Center database
- [ ] Allocate credits to organization
