...
curl -X GET 'https://unicorncommander.ai/auth/login'

# Then test endpoints (cookie automatically included)
curl -X GET 'https://unicorncommander.ai/api/v1/admin/users?search=aaron&max=10' \
...
- **Keycloak**: Admin REST API at `https://auth.unicorncommander.ai`
- **Python Packages**:
...
# Usage
client = OpsAdminClient("https://unicorncommander.ai", session_token)
