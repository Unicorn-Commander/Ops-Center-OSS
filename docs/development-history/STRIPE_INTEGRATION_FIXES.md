...
**Files Changed**:
- `/opt/ops-center/backend/subscription_manager.py`
...
**Files Changed**:
- `/opt/ops-center/backend/stripe_integration.py`
...
**Files Changed**:
- `/opt/ops-center/backend/stripe_api.py`
...
3. **Plan Retrieval**:
   ...
   curl https://unicorncommander.ai/api/v1/billing/plans | jq '.[] | {id, monthly: .stripe_price_id, annual: .stripe_annual_price_id}'
   ```
...
2. **Verify Environment Variables** ✅ REQUIRED
   ```bash
   # Check .env.auth file
   grep STRIPE_SECRET_KEY /opt/ops-center/.env.auth
...
```bash
STRIPE_SECRET_KEY=sk_test_your_stripe_secret_key  # Test mode key
