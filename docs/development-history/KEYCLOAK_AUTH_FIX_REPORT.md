...
```bash
TOKEN=$(curl -s "https://auth.unicorncommander.ai/realms/master/..." | jq -r .access_token)
curl -H "Authorization: Bearer $TOKEN" \
  "https://auth.unicorncommander.ai/admin/realms/uchub/users?max=1"
...
2. ✅ **Master Realm Authentication**
   ```bash
   curl -X POST "https://auth.unicorncommander.ai/realms/master/..."
   ```
   - Result: 200 OK
...
3. ✅ **UCHUB Realm Authentication**
   ```bash
   curl -X POST "https://auth.unicorncommander.ai/realms/uchub/..."
   ```
...
4. ✅ **Admin API Access**
   ...
   curl -H "Authorization: Bearer <master-token>" \
     "https://auth.unicorncommander.ai/admin/realms/uchub/users"
   ```
