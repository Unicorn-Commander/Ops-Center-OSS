...
class FederationMeter:
    ...
    def __init__(
        ...
        *,
        ...
        # Canonical billable metric — identical contract to the gateway's
        # Lago callback. Verified live 2026-06-11: the gateway emits RAW
        # response_cost in the event; the ×1.32 never-lose-money markup is
        # applied downstream at the Lago plan/charge level. Default markup
        # here is therefore 1.0 (raw) — only set FEDERATION_LAGO_MARKUP when
        # a deployment's billing plan does NOT apply the markup itself,
        # otherwise the same usage would bill 1.32× differently depending on
        # which path metered it.
        self.lago_event_code = os.getenv("FEDERATION_LAGO_EVENT_CODE", "ai_api_call")
        self.lago_markup = float(os.getenv("FEDERATION_LAGO_MARKUP", "1.0"))
    ...
    async def record_usage(self, usage_event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Normalize event
        metadata = dict(usage_event.get("metadata", {}) or {})
        org_id = usage_event.get("org_id") or metadata.get("org_id")
        gateway_metered = bool(
            usage_event.get("gateway_metered", metadata.get("gateway_metered", False))
        )
        if org_id:
            metadata["org_id"] = org_id
        metadata["gateway_metered"] = gateway_metered
        # Optional unit-based billable metric for non-LLM local compute
        # (embeddings/reranking/STT/TTS). When set, _report_to_lago emits THIS
        # metric code with the unit quantity instead of the cost-based
        # `ai_api_call`. The LLM path leaves these unset → unchanged behavior.
        billing_metric = usage_event.get("billing_metric")
        billing_quantity = usage_event.get("billing_quantity")
        billing_unit = usage_event.get("billing_unit")
        if billing_metric:
            metadata.setdefault("billing_metric", billing_metric)
            metadata.setdefault("billing_quantity", billing_quantity)
            metadata.setdefault("billing_unit", billing_unit)
        event = {
            "event_id": event_id,
        ...
        # Report to Lago (non-blocking) — UNLESS the request was already
        # metered by a gateway. A federated call served under a per-org
        # gateway key fires its (single) billable Lago event from the
        # publisher's gateway; firing here too would double-bill.
        if gateway_metered:
            logger.debug(
                "Skipping Lago report for %s — already gateway-metered (org=%s)",
                event_id, org_id,
            )
        elif self.lago_url and self.lago_api_key:
            try:
    ...
    async def _report_to_lago(self, event: Dict[str, Any]) -> None:
        ...
        Two shapes, ONE billable metric:

        - Org-attributed event (org_id known, NOT gateway-metered): emit the
          canonical `ai_api_call` metric keyed external_subscription_id ==
          org_id with response_cost = cost_usd × markup — the same contract
          the LiteLLM gateway fires, so org billing is uniform regardless of
          which path metered the call.
        - Node-level event (no org identity): legacy `federation_<service>`
          code keyed by source node slug. This is NOT the billable metric —
          it exists for node-to-node settlement visibility only.
        """
        ...
        billing_metric = event.get("billing_metric")
        if org_id and billing_metric:
            # Non-LLM unit-based compute (embeddings / reranking / STT / TTS):
            # emit the dedicated metric code with the unit quantity. The Lago
            # plan charge converts units → price; response_cost is carried for
            # cost dashboards but the billable dimension is the unit count.
            quantity = event.get("billing_quantity") or 0
            unit = event.get("billing_unit") or "units"
            props = {
                "service_type": event.get("service_type", ""),
                "model": event.get("model", ""),
                "units": quantity,
                "response_cost": round(cost * self.lago_markup, 10),
                "cost_usd": str(cost),
                "via": "federation_meter",
            }
            # Also expose the count under its named unit (e.g. "tokens",
            # "characters", "audio_seconds", "searches") for Lago metric
