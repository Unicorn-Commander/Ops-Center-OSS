...
class InferenceRouter:
    ...
    def _get_trust_enforcer(self) -> Any:
        """Trust-mode enforcer (federation_peers.trust_mode/publish/consume).

        Injectable for tests; otherwise resolved from the module singleton,
        late-binding the registry's db_pool so policies load from PostgreSQL.
        """
        if self._trust_enforcer is None:
            try:
                from federation.trust import get_trust_enforcer
                self._trust_enforcer = get_trust_enforcer(
                    getattr(self.registry, "db_pool", None),
                    local_node_id=self.local_node_id,
                )
            except Exception as exc:  # never let trust wiring break routing
                logger.error("Trust enforcer unavailable: %s", exc)
    ...
    async def proxy_to_node(
        ...
        *,
        ...
        Args:
            ...
            org_id: Consuming org identity. When set, it is carried in the
                X-Federation-Org-Id header so the publisher's gateway meters
                the call under that org's per-org virtual key — the
                publisher fires the (single) billable Lago event, so our own
                meter records this hop as gateway_metered.
        ...
        # Carry the consuming org identity so the publisher's gateway meters
        # under the org's per-org virtual key (see federation/gateway_bridge).
        if org_id:
            request_headers["X-Federation-Org-Id"] = org_id
        ...
        try:
            ...
            # Record usage if metering is available. When the call carried an
            # org identity, the PUBLISHER's gateway fired the billable Lago
            # event — mark gateway_metered so FederationMeter only logs it
            # locally and never double-fires Lago.
            if self.meter:
                try:
    ...
    async def proxy_to_node_stream(
        self,
        node_id: str,
        endpoint_path: str,
        request_data: Dict[str, Any],
        *,
        ...
        """Streaming sibling of proxy_to_node — yields raw response bytes.

        Same node resolution, circuit breaker, auth and org-identity header as
        proxy_to_node, but opens a streaming POST and relays the peer's SSE
        chunks verbatim. The PUBLISHER's gateway meters the call on stream
        completion (gateway_metered), so we record only a local usage log.
        """
        circuit_breaker = get_circuit_breaker()
        if not circuit_breaker.can_request(node_id):
            raise ConnectionError(
        ...
        try:
            async with httpx.AsyncClient(timeout=self._http_timeout) as client:
                async with client.stream(
                    "POST", url, json=request_data, headers=request_headers
                ) as response:
            ...
            if self.meter:
                try:
                    ...
                    logger.warning("Failed to record stream usage: %s", meter_exc)
        except Exception as exc:
            circuit_breaker.record_failure(node_id)
            logger.error("Proxy stream to %s failed: %s", node_id, exc)
            raise
    ...
    async def _find_peer_services(
        ...
        Candidates are filtered through the trust-mode enforcer: a peer is
        only routable if our federation_peers row for it allows outbound
        consumption of this service type (full/consumer = all, scoped =
        consume[] ACL only, publisher/isolated = nothing). Denials are
        written to the federation_routing_audit table.
        """
        all_services = await self.registry.get_service_catalog(
            service_type=service_type
        )
        enforcer = self._get_trust_enforcer()
        audit_log = get_routing_audit_log()
        candidates = []
        for svc in all_services:
            ...
            if model and model not in (svc.get("models") or []):
                continue
            # Trust-mode gate (deny-by-default inside scoped ACLs)
            if enforcer is not None:
                peer_id = svc.get("node_id", "")
                allowed, reason = await enforcer.we_may_consume(peer_id, service_type)
                if not allowed:
                    logger.info(
                        "Trust mode blocked peer candidate %s for %s: %s",
                        peer_id, service_type, reason,
                    )
                    await audit_log.log_decision(
                        request_id=str(uuid.uuid4()),
                        service_type=service_type, model=model,
                        user_id=None, user_tier=None,
                        candidates_found=1, candidates_after_constraints=0,
                        selected_target="peer", selected_node_id=peer_id,
