...
from billing_config_api import router as billing_config_router
from pricing_rates_api import router as pricing_rates_router
from litellm_api import router as litellm_api_router  # Chat completions, credits, BYOK
from help_guide_api import router as help_guide_router  # The Guide — end-user help agent (no system access)
...
from system_settings_api import router as system_settings_admin_router
from notification_preferences_api import router as notification_preferences_router
from metered_compute_api import router as metered_compute_router
from internal_llm_preflight_api import router as internal_llm_preflight_router
from paid_org_provisioning import router as paid_org_provisioning_router
...
# Inference Proxy (Majiks Studio, Artwork Studio, Whisper) - March 2026
from routers.inference_proxy import router as inference_proxy_router

# GPU Compute Fabric Control Plane - April 2026
from routers.compute_api import router as compute_api_router
try:
    from routers.uuda import router as uuda_router
except ImportError:
    uuda_router = None

# Billing Events API - April 2026
from routers.billing_events import router as billing_events_router
...
from entitlements_api import router as entitlements_router
...
# Federation Settings API (March 2026) - GUI configuration for federation
from federation_settings_api import router as federation_settings_router
from federation_contracts_api import router as federation_contracts_router
from schema_drift_api import router as schema_drift_router
from federation.startup import start_federation_agent, stop_federation_agent
# Admin GUI: pricing/policy config (June 2026)
from app_model_policy_api import router as app_model_policy_router
from credit_packages_admin_api import router as credit_packages_admin_router
from federation_agent_pricing_api import router as federation_agent_pricing_router
...
from llm_keys import router as llm_keys_router
from gateway_key_provisioning import router as gateway_key_router
from vpn_keys import router as vpn_keys_router
...
async def get_user_org_context(user_id: str, email: str = None) -> dict:
    ...
    async def check_database_membership(uid: str) -> dict:
        ...
        try:
            ...
            if hasattr(app.state, 'db_pool') and app.state.db_pool:
                async with app.state.db_pool.acquire() as conn:
                    ...
                    # Also join subscription_tiers to get the tier_code for apps access
                    result = await conn.fetchrow("""
                        SELECT om.org_id, om.role, o.name as org_name,
                               st.tier_code as subscription_tier
                        FROM users u
                    ...
                    if result:
                        logger.info(f"Found org membership in database for Keycloak user {uid}: {result['org_id']} (tier: {result.get('subscription_tier', 'unknown')})")
                        return {
                    ...
                    if result:
                        ...
                        return {
                            ...
                            "org_name": result["org_name"],
                            "org_role": result["role"].lower() if result["role"] else "member",
                            "subscription_tier": result["subscription_tier"] or "trial"
                        }
            else:
        ...
        return {"org_id": None, "org_name": None, "org_role": None, "subscription_tier": "trial"}
    ...
    try:
        ...
        return {
            ...
            "org_name": org_name,
            "org_role": org_role,
            "subscription_tier": "trial"  # Default for Keycloak-based lookup
        }
...
async def auto_provision_user_org(user_id: str, email: Optional[str], display_name: str) -> dict:
    ...
    # Provision the per-org gateway key (metered-inference billing keystone).
    # Best-effort + failure-isolated: never blocks provisioning.
    try:
        from gateway_key_provisioning import provision_org_gateway_key
        await provision_org_gateway_key(org_id=org_id, plan_code="free")
    except Exception as gw_error:
        logger.error(f"Failed to provision gateway key for org {org_id}: {gw_error}")
...
csrf_protect, csrf_middleware_factory = create_csrf_protection(
    ...
    exempt_urls={
        ...
        "/api/v1/auth/validate-key",  # Machine-to-machine key validation for mesh services
        "/api/v1/auth/profile",  # Profile update - session-authenticated, multipart form upload
        "/api/v1/webhooks/lago",  # Lago webhooks
        "/api/v1/webhooks/stripe",  # Stripe webhooks (signature-verified, not session/CSRF)
        ...
        "/api/v1/metered/",  # Metered compute proxy/bridge - API/service auth, not forms
        "/api/v1/network/vpn-keys",  # VPN pre-auth key management (admin, REST API)
        "/api/v1/network/nodes",      # VPN node management (admin, REST API)
        ...
        "/a2a",  # A2A JSON-RPC endpoint - external service-to-service calls
        "/.well-known/",  # A2A agent card discovery - public protocol endpoint
        "/api/v1/federation/",  # Federation API - machine-to-machine, uses API key auth
        "/api/v1/admin/federation/",  # Federation settings API - admin auth, not CSRF
        "/api/v1/admin/pricing/",  # Pricing rate-book API - admin auth via require_admin_user, not CSRF
        "/api/v1/inference/",  # Inference proxy - programmatic API, session auth not CSRF
        "/api/v1/compute/",  # GPU Compute Fabric - machine-to-machine, uses agent key auth
        "/api/v1/billing/events",  # Billing Events API - machine-to-machine, uses agent key auth
        "/api/v1/internal/llm/",  # Gateway local-pricing preflight - service-key m2m, not forms
        "/api/v1/uuda/",  # UUDA control plane
        "/api/v1/branding/config",  # Public runtime branding endpoint (no auth required)
    },
...
# Default-deny Authentication Middleware (2026-06-08 audit remediation; customer-node
# hotfixed 2026-06-09). Gates every /api/* request that isn't on the public/m2m allowlist,
# requiring a valid session — so a router that forgets its per-endpoint auth dependency
# fails CLOSED instead of silently serving data unauthenticated.
# Kill switch: AUTH_MIDDLEWARE_ENABLED=false (then recreate).
from auth_middleware import create_auth_middleware
AUTH_MIDDLEWARE_ENABLED = os.getenv("AUTH_MIDDLEWARE_ENABLED", "true").lower() == "true"
app.add_middleware(create_auth_middleware(enabled=AUTH_MIDDLEWARE_ENABLED))
logger.info(f"Auth Middleware: {'Enabled' if AUTH_MIDDLEWARE_ENABLED else 'Disabled'} (default-deny on /api/*)")
...
async def startup_event():
    ...
    try:
        ...
        # Ensure feature tables exist (idempotent). Many handlers query tables that
        # ship as raw .sql and were never auto-applied, causing 500s on a fresh node
        # (this is what broke ~22 admin endpoints on a clean deploy). Apply the
        # consolidated idempotent migration on every boot — CREATE ... IF NOT EXISTS,
        # so it's a no-op once present. Non-fatal: a failure here must not block startup.
        try:
            _eft_path = os.path.join(os.path.dirname(__file__), "migrations", "ensure_feature_tables.sql")
            if os.path.exists(_eft_path):
                with open(_eft_path) as _f:
                    _eft_sql = _f.read()
                # Execute statement-by-statement so a single failing statement
                # (e.g. a column-parity mismatch against a pre-existing table on
                # one node) does NOT roll back the ENTIRE migration — asyncpg runs
                # a multi-statement string as one implicit transaction. Respect
                # $$-dollar-quoted bodies (functions / DO blocks) when splitting.
                _stmts, _buf, _in_dollar = [], [], False
                for _line in _eft_sql.splitlines():
                    _buf.append(_line)
                    if _line.count("$$") % 2 == 1:
                        _in_dollar = not _in_dollar
                    if not _in_dollar and _line.rstrip().endswith(";"):
                        _stmts.append("\n".join(_buf)); _buf = []
                if any(s.strip() for s in _buf):
                    _stmts.append("\n".join(_buf))
                _eft_ok = _eft_fail = 0
                async with db_pool.acquire() as _conn:
                    for _stmt in _stmts:
                        if not _stmt.strip():
                            continue
                        try:
                            await _conn.execute(_stmt)
                            _eft_ok += 1
                        except Exception as _se:
                            _eft_fail += 1
                            logger.warning(f"ensure_feature_tables: statement skipped: {str(_se)[:140]}")
                logger.info(f"ensure_feature_tables.sql applied ({_eft_ok} ok, {_eft_fail} skipped)")
            else:
                logger.warning(f"ensure_feature_tables.sql not found at {_eft_path}; skipping")
        except Exception as e:
            logger.error(f"ensure_feature_tables.sql failed (non-fatal, continuing startup): {e}")

        # Versioned core-table migration parity + schema-drift self-check (2026-06-13).
        # Both deploy branches run the SAME migrations/core/*.sql here so cherry-picked
        # code can't assume a column the other node lacks; then we compare the live
        # CORE tables to the canonical contract and log LOUDLY on drift (deploy-time
        # signal instead of a customer-facing 500). Both steps are non-fatal.
        try:
            import core_migrations
            import schema_drift
            _cm = await core_migrations.run_core_migrations(db_pool)
            if _cm.get("applied"):
                logger.info(f"core_migrations applied: {[a['version'] for a in _cm['applied']]}")
            _drift = await schema_drift.check_drift(db_pool)
            schema_drift.log_report(_drift)
        except Exception as e:
            logger.error(f"core_migrations/schema_drift failed (non-fatal, continuing startup): {e}")

        # === migrations runner (Phase 0) ===
        # Manifest-driven FEATURE-schema convergence (T3, 2026-07-02). Applies the
        # curated idempotent migrations/*.sql files listed in migrations/manifest.json
        # (model lists, org features, ...) so feature tables converge on every node
        # without hand-applied SQL — the layer between the ensure_feature_tables
        # self-heal above and the versioned core migrations. Statement-tolerant and
        # NON-FATAL: startup must never crash because of a migration.
        try:
            import migrations_runner
            _mr = await migrations_runner.run_manifest_migrations(db_pool)
            if _mr.get("applied"):
                logger.info(f"migrations_runner applied: {[a['name'] for a in _mr['applied']]}")
            if _mr.get("failed"):
                logger.error(f"migrations_runner reported failures (non-fatal): {_mr['failed']}")
        except Exception as e:
            logger.error(f"migrations_runner failed (non-fatal, continuing startup): {e}")
        # === end migrations runner (Phase 0) ===
        ...
        # Initialize System Settings manager (admin CRUD at /api/v1/admin/system-settings).
        # ensure_tables() self-heals admin_system_settings(+_audit) on hosts that
        # never ran a migration for them (CREATE TABLE IF NOT EXISTS, idempotent).
        try:
            from system_settings_manager import SystemSettingsManager
            from system_settings_api import set_settings_manager
            _settings_mgr = SystemSettingsManager(db_pool, redis_client)
            await _settings_mgr.ensure_tables()
            set_settings_manager(_settings_mgr)
            logger.info("System Settings manager initialized (admin_system_settings tables ensured)")
        except Exception as e:
            # Non-fatal: only the settings CRUD endpoints 500, not the whole API
            logger.error(f"Failed to initialize System Settings manager: {e}")
    ...
    # Scheduled pricing refresh — NEVER-LOSE-MONEY safeguard: keep the metered-inference
    # rate book's provider COSTS current from live OpenRouter pricing, so a provider price
    # hike can't sit unbilled between manual refreshes. Runs once ~30s after boot (every
    # deploy picks up current prices) then every PRICING_REFRESH_HOURS (default 8 = 3x/day).
    # Hikes exceeding the safety buffer are logged + surfaced via /admin/pricing/status.
    try:
        PRICING_REFRESH_ENABLED = os.getenv("PRICING_REFRESH_ENABLED", "true").lower() == "true"
        PRICING_REFRESH_HOURS = float(os.getenv("PRICING_REFRESH_HOURS", "8") or 8)
        if PRICING_REFRESH_ENABLED:
            async def _pricing_refresh_loop():
                from pricing_rates_api import perform_refresh
                await asyncio.sleep(30)  # let the app settle before first pull
                while True:
                    try:
                        await perform_refresh(reason="scheduled")
                    except Exception as e:
                        logger.error(f"Scheduled pricing refresh failed (non-fatal): {e}")
                    await asyncio.sleep(PRICING_REFRESH_HOURS * 3600)
            asyncio.create_task(_pricing_refresh_loop())
            logger.info(f"Scheduled pricing refresh enabled (every {PRICING_REFRESH_HOURS}h)")
        else:
            logger.info("Scheduled pricing refresh disabled (PRICING_REFRESH_ENABLED=false)")
    except Exception as e:
        logger.error(f"Failed to start scheduled pricing refresh: {e}")
        # Don't block startup if the scheduler can't start

    # Register Colonel with Brigade (fire-and-forget background task)
    try:
        from colonel.a2a_server import register_with_brigade
        asyncio.create_task(register_with_brigade())
        logger.info("Colonel Brigade registration task launched (background)")
    except Exception as e:
        logger.warning(f"Could not launch Colonel Brigade registration: {e}")
        # Don't block startup — Colonel works standalone

    # Start federation agent (heartbeat, discovery, peering)
    try:
        await start_federation_agent()
        logger.info("Federation agent started successfully")
    except Exception as e:
        logger.warning(f"Could not start federation agent: {e}")
        # Don't block startup — federation is optional
...
async def shutdown_event():
    ...
    try:
        ...
        logger.info("Federation agent stopped")
    except Exception as e:
...
# Register The Guide (end-user help agent — no system access, internal/local model)
app.include_router(help_guide_router)
logger.info("The Guide help-agent endpoint registered at /api/v1/help/guide")
...
app.include_router(pricing_rates_router)
...
# Federation mesh management (March 2026)
app.include_router(federation_router)
logger.info("Federation API endpoints registered at /api/v1/federation")

# Core-table schema-drift check (2026-06-13)
app.include_router(schema_drift_router)
logger.info("Schema-drift API endpoints registered at /api/v1/admin/schema-drift")
...
# System Settings admin CRUD (encrypted, audited; manager wired in startup_event)
app.include_router(system_settings_admin_router)
logger.info("System Settings admin CRUD registered at /api/v1/admin/system-settings")

# Notification Preferences API (real, persisted — replaces the mock GET above)
app.include_router(notification_preferences_router)
logger.info("Notification Preferences API endpoints registered")

# Metered Compute API (Lago billing for embeddings/reranking/STT/TTS)
app.include_router(metered_compute_router)
app.include_router(internal_llm_preflight_router)
app.include_router(paid_org_provisioning_router)
logger.info("Metered Compute API endpoints registered at /api/v1/metered")
...
logger.info("Inference Proxy endpoints registered at /api/v1/inference")

# GPU Compute Fabric Control Plane - April 2026
app.include_router(compute_api_router)
logger.info("GPU Compute Fabric endpoints registered at /api/v1/compute")

# UUDA control plane (Phase 1) — April 2026
if uuda_router is not None:
    app.include_router(uuda_router)
    logger.info("UUDA control-plane endpoints registered at /api/v1/uuda")


# Billing Events API - April 2026
app.include_router(billing_events_router)
logger.info("Billing Events API endpoints registered at /api/v1/billing/events")

# Federation Settings API (Admin GUI) - March 2026
app.include_router(federation_settings_router)
logger.info("Federation Settings API endpoints registered at /api/v1/admin/federation")
app.include_router(federation_contracts_router)
logger.info("Federation Contracts API endpoints registered at /api/v1/admin/federation/contracts")

# Admin GUI: pricing/policy config (June 2026)
app.include_router(app_model_policy_router)
app.include_router(credit_packages_admin_router)
app.include_router(federation_agent_pricing_router)
logger.info("Admin pricing/policy API endpoints registered (inference-policy, credit-packages, agent-pricing)")
...
logger.info("LLM Keys endpoints registered at /api/v1/llm/keys")
app.include_router(gateway_key_router)
logger.info("Org Gateway Key endpoint registered at /api/v1/org/gateway-key (per-org metered-inference key)")
app.include_router(vpn_keys_router)
logger.info("VPN Keys endpoints registered at /api/v1/network (vpn-keys, nodes)")
...
# NOTE: these supplementary endpoints have no real data source yet. They now return
# honest empty payloads (same JSON shape, zeroed values, "no_data": true) instead of
# the fabricated random numbers they used to serve. Registration order is unchanged
# so response shapes stay stable for existing consumers.
app.include_router(analytics_revenue_router)
app.include_router(analytics_users_router)  # PRIORITY: overrides old DB endpoints (cohorts, growth) with stable-shape no_data payloads
...
logger.info("Supplementary Analytics endpoints registered (PRIORITY - honest no_data payloads until real queries land):")
...
async def get_current_user(request: Request):
    ...
    # Ensure user_id exists (Keycloak sessions carry 'sub', not 'user_id'). Many handlers
    # index current_user["user_id"] directly, which 500'd with KeyError: 'user_id' (e.g.
    # /api/v1/api-keys, /api/v1/sessions). Map it from sub/id here so every caller is safe.
...
async def get_system_settings():
    ...
    return {
        ...
        "branding": {
            "company_name": branding.get("company_name", "Ops-Center"),
            "logo_url": branding.get("logo_url", "/logos/ops-center-logo.png"),
            "primary_color": branding.get("primary_color", "#7c3aed")
        },
        "sso_enabled": branding.get("sso_enabled", True),
...
async def get_deployment_config():
    ...
    try:
        if DEPLOYMENT_CONFIG:
            ...
            # Fallback configuration — uses runtime branding
            runtime_branding = get_branding_config()
            config = {
                ...
                "branding": {
                    "name": runtime_branding.get("company_name", "Ops-Center"),
                    "theme": runtime_branding.get("theme", "default")
                }
            }
        return config
    except Exception as e:
...
# ===========================
@app.get("/api/v1/branding/config")
async def get_branding_config_public():
    """Get runtime branding configuration for the current tenant.
    Public endpoint (no authentication required) so the login page
    and unauthenticated visitors render with correct branding.
    """
    try:
        branding = get_branding_config()
        return branding
...
def build_keycloak_logout_url(logout_confirmation_url: str, id_token: Optional[str] = None) -> str:
    """Build a host-aware Keycloak end-session URL for this branded deployment."""
    from urllib.parse import urlencode

    external_host = os.getenv("EXTERNAL_HOST", EXTERNAL_HOST)
    external_protocol = os.getenv("EXTERNAL_PROTOCOL", EXTERNAL_PROTOCOL)
    keycloak_external_url = os.getenv("KEYCLOAK_EXTERNAL_URL", f"{external_protocol}://auth.{external_host}")
    keycloak_realm = os.getenv("KEYCLOAK_REALM", "uchub")
    client_id = os.getenv("KEYCLOAK_CLIENT_ID") or os.getenv("OAUTH_CLIENT_ID", "ops-center")
    ...
    return (
        f"{keycloak_external_url.rstrip('/')}/realms/{keycloak_realm}"
        f"/protocol/openid-connect/logout?{urlencode(params)}"
    )


def delete_session_cookie(response: Response) -> None:
    """Clear both host-only and shared-domain session cookies."""
    response.delete_cookie("session_token", path="/")
    external_host = os.getenv("EXTERNAL_HOST", EXTERNAL_HOST)
    if "." in external_host and not external_host.startswith("localhost"):
        response.delete_cookie("session_token", path="/", domain=f".{external_host}")
...
async def logout(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        keycloak_logout_url = build_keycloak_logout_url(logout_confirmation_url, id_token)
        ...
        response = JSONResponse(content={
            ...
            "sso_logout_url": keycloak_logout_url
        })
...
async def get_me(current_user: dict = Depends(get_current_user)):
    ...
    The session user dict may be a local-account shape (user_id) or a
    Keycloak/SSO shape (sub/email/preferred_username). Resolve the id from
    whatever the session has, look it up in the local user store, and if the
    user is not in the local store (SSO users won't be) or the lookup fails,
    fall back to returning the session user dict which already carries the
    authenticated identity. This must never 500 -- the Revenue page depends on it.
    """
    current_user = current_user or {}
    # Try the common id keys in priority order
    user_id = (
        current_user.get("user_id")
        or current_user.get("sub")
        or current_user.get("id")
        or current_user.get("keycloakId")
        or current_user.get("email")
    )
    try:
        user = auth_manager.get_user(user_id) if user_id else None
    except Exception as e:
        logger.warning(f"get_me: auth_manager.get_user failed for '{user_id}': {e}")
        user = None

    # Fall back to the session user dict (already contains the authenticated info)
...
async def update_profile(
    ...
    try:
        ...
        if avatar and avatar.filename:
            ...
            # Persist the avatar URL durably so it survives session expiry / re-login.
            # The session copy below is still kept for the fast path; this row is the
            # source of truth that get_session_info + the OAuth callback rehydrate from.
            try:
                if hasattr(app.state, 'db_pool') and app.state.db_pool:
                    async with app.state.db_pool.acquire() as conn:
                        await conn.execute(
                    ...
                    logger.info(f"Persisted avatar_url for {user_email}")
                else:
                    logger.warning("Database pool not available; avatar_url not persisted durably")
            except Exception as avatar_db_err:
                # Non-fatal: the file is saved and the session is updated; durability
                # is best-effort so a DB hiccup doesn't break the profile update.
                logger.error(f"Failed to persist avatar_url for {user_email}: {avatar_db_err}")
...
@app.post("/api/v1/auth/validate-key")
async def validate_key_endpoint(request: Request):
    """
    Validate a UC API key and return user context for downstream services.

    Used by mesh services (Center Deep Search, etc.) to authenticate requests
    that carry a user's UC API key. Returns user identity, subscription tier,
    organization, and role so the caller can apply its own tier/quota policy.

    Request body: {"key": "uc_..."}

    Responses:
      200 {"valid": true, "user": {...}}   valid key
      200 {"valid": false}                 invalid key
    ...
    api_key = body.get("key") if isinstance(body, dict) else None
    ...
    try:
        ...
        logger.error(f"validate-key: api_key_manager error: {e}", exc_info=True)
    ...
    try:
        ...
        logger.warning(f"validate-key: org lookup failed for user {user_id}: {e}")

    effective_tier = None
    if org_info:
        effective_tier = org_info.get("subscription_tier") or org_info.get("plan_tier")
...
async def get_network_configuration(current_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        # Degrade gracefully: in a container (no netplan / restricted host
        # network files) full discovery may fail. Return a valid NetworkStatus
        # populated with whatever is available instead of 500ing the page.
        logger.warning(f"Network configuration partial read (degraded): {e}")
        import socket
        try:
...
async def register_user(request: Request):
    ...
    7. Subscribe organization to "free" plan
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Successfully subscribed org {org_id} to free plan")
        ...
        # Step 5.1: Provision the per-org gateway key (metered-inference billing
        # keystone). Best-effort + failure-isolated: never fails registration.
        try:
            from gateway_key_provisioning import provision_org_gateway_key
            await provision_org_gateway_key(org_id=org_id, plan_code="free")
        except Exception as gw_error:
...
async def direct_login(request: Request, credentials: dict):
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Federated deployments (e.g. GFL) set KEYCLOAK_EXTERNAL_URL to point at the shared Keycloak
        if "yoda.unicorncommander.ai" in str(request.url):
            auth_url = "https://auth.yoda.unicorncommander.ai"
        else:
            auth_url = os.getenv("KEYCLOAK_EXTERNAL_URL", f"https://auth.{EXTERNAL_HOST}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if user_response.status_code == 200:
                    ...
                    # Rehydrate the durable profile picture from users.avatar_url so it
                    # survives re-login (Keycloak claims don't carry it). Stored into
                    # the session user object below.
                    if email and not user_info.get("avatar"):
                        try:
                            if hasattr(app.state, 'db_pool') and app.state.db_pool:
                                async with app.state.db_pool.acquire() as conn:
                                    saved_avatar = await conn.fetchval(
                                        "SELECT avatar_url FROM users WHERE email = $1",
                                        email
                                    )
                                if saved_avatar:
                                    user_info["avatar"] = saved_avatar
                        except Exception as avatar_hydrate_err:
                            logger.error(f"avatar_url hydration failed for {email}: {avatar_hydrate_err}")
...
async def oauth_login(request: Request):
    """Redirect to Keycloak, preserving the requested post-login destination."""
    state = secrets.token_urlsafe(32)
    return_url = request.query_params.get("return_url")
    signup = request.query_params.get("signup") == "1"
    sessions[state] = {"created": time.time(), "return_url": return_url, "signup": signup}
    ...
    if "yoda.unicorncommander.ai" in str(request.url):
        ...
        # Federated deployments (e.g. GFL) set KEYCLOAK_EXTERNAL_URL to point at the shared Keycloak
        auth_base = os.getenv("KEYCLOAK_EXTERNAL_URL", f"https://auth.{EXTERNAL_HOST}")

    # Keycloak realm (default to "master" if not specified)
    keycloak_realm = os.getenv("KEYCLOAK_REALM", "master")

    # Use Keycloak OAuth endpoints (not Authentik)
    auth_url = (
        f"{auth_base}/realms/{keycloak_realm}/protocol/openid-connect/auth"
...
async def login_google(request: Request):
    ...
    authorize_url = (
        f"{auth_url}/realms/{keycloak_realm}/protocol/openid-connect/auth"
        f"?client_id={OAUTH_CLIENT_ID}"
        f"&redirect_uri={redirect_uri}"
        f"&response_type=code"
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Get user info using external Keycloak URL
                # Use external URL to ensure JWT validation succeeds
                # Fall back to auth.{EXTERNAL_HOST} for deployments that host their own Keycloak;
                # federated deployments (e.g. GFL) must set KEYCLOAK_EXTERNAL_URL explicitly
                keycloak_external_url = os.getenv("KEYCLOAK_EXTERNAL_URL", f"https://auth.{EXTERNAL_HOST}")
                userinfo_url = f"{keycloak_external_url}/realms/{keycloak_realm}/protocol/openid-connect/userinfo"
                ...
                if user_response.status_code == 200:
                    ...
                    # Auto-provision org for first-time SSO users with no org context
                    # (mirrors direct_login flow at line ~5615 — federated SSO users
                    # arriving via auth.<domain>/realms/uchub need a local org row)
                    user_role_for_provision = user_info.get("role", "viewer")
                    if not org_context.get("org_id") and user_role_for_provision != "admin":
                        display_name_for_provision = (
                            user_info.get("name")
                            or user_info.get("preferred_username")
                            or username
                        )
                        try:
                            org_context = await auto_provision_user_org(
                                user_id=user_id,
                                email=email,
                                display_name=display_name_for_provision
                            )
                            auto_provisioned = True
                            logger.info(f"SSO auto-provisioned org for user {email}")
                        except Exception as provision_error:
                            logger.error(f"SSO auto-provisioning failed for {email}: {provision_error}")
                    ...
                    sessions[session_token] = {
                        ...
                        "org_name": org_context.get("org_name"),
                        "org_role": org_context.get("org_role"),
                        "subscription_tier": org_context.get("subscription_tier", "trial")  # For apps access control
                    }
                    ...
                    # A storefront-selected return URL takes precedence over the
                    # legacy signup-flow redirect. Accept same-site relative paths
                    # and HTTPS URLs on this deployment's host only.
                    if stored_return_url:
                        safe_relative = stored_return_url.startswith("/") and not stored_return_url.startswith("//")
                        safe_absolute = (
                            stored_return_url.startswith("https://")
                            and EXTERNAL_HOST in stored_return_url
                        )
                        if safe_relative or safe_absolute:
...
async def logout(request: Request):
    """Logout, clear local session, and continue through Keycloak SSO logout."""
    session_token = request.cookies.get("session_token")
    id_token = None
    ...
    external_protocol = os.getenv("EXTERNAL_PROTOCOL", EXTERNAL_PROTOCOL)
    logout_confirmation_url = f"{external_protocol}://{external_host}/auth/logged-out"
    response = RedirectResponse(url=build_keycloak_logout_url(logout_confirmation_url, id_token))
    delete_session_cookie(response)
    return response
...
async def logged_out(request: Request):
    ...
    brand_name = escape(os.getenv("BRANDING_PAGE_TITLE") or os.getenv("BRANDING_COMPANY_NAME") or "Ops-Center")
    html_content = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <title>Signed Out - """ + brand_name + """</title>
        <style>
    ...
    <body>
        <div class="card">
            ...
            <h1>Signed Out</h1>
            <p>Your Ops-Center session has ended. This page will stay here so the browser does not immediately start a new SSO login.</p>
            <div class="actions">
                <a class="button" href="/auth/login">Sign In Again</a>
                <a class="button secondary" href="/">Return Home</a>
            </div>
            <p class="note">If another tab is still open, refresh it before continuing.</p>
        </div>
    </body>
    </html>
    """
...
async def get_session_info(request: Request):
    ...
    if session_token and session_token in sessions:
        ...
        # Resolve the profile picture: prefer the session copy (fast path, set on
        # upload / login), otherwise fall back to the durable users.avatar_url row so
        # the picture survives session expiry. Without this, data.user.avatar is null
        # on reload and the frontend preview resets.
        avatar_value = user_info.get("avatar")
        if not avatar_value:
            ...
            if user_email:
                try:
                    if hasattr(app.state, 'db_pool') and app.state.db_pool:
                        async with app.state.db_pool.acquire() as conn:
                            avatar_value = await conn.fetchval(
                                "SELECT avatar_url FROM users WHERE email = $1",
                                user_email
                            )
                except Exception as avatar_lookup_err:
                    logger.error(f"avatar_url lookup failed for {user_email}: {avatar_lookup_err}")
...
async def get_sso_token(request: Request):
    ...
    This endpoint allows trusted subdomains (*.magicunicorn.dev) to obtain
...
async def serve_admin(request: Request, path: str = ""):
    ...
    Serves the SPA unconditionally (no auth redirect).
    React's ProtectedRoute component handles auth checks client-side.
    The login page at /admin/login renders with runtime branding for
    unauthenticated users, showing a branded SSO login button.
    """
    # Serve the SPA to ALL users (authenticated or not)
    # React's ProtectedRoute component handles auth checks client-side
    if os.path.exists("public/index.html"):
        response = FileResponse("public/index.html")
        response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
