...
async def get_current_user(request: Request):
    ...
    if auth_header.lower().startswith("bearer "):
        ...
        # The SPA stores the Redis session token in localStorage and sends it as
        # a bearer. Try Redis session lookup first; fall back to JWT verify for
        # tokens issued by auth_manager (local users).
        try:
            sessions = RedisSessionManager(
                host=os.getenv("REDIS_HOST", "unicorn-redis"),
                port=int(os.getenv("REDIS_PORT", "6379")),
        ...
        try:
            jwt_user = auth_manager.verify_token(token)
        except Exception:
            jwt_user = None
        if not jwt_user:
            raise HTTPException(status_code=401, detail="Invalid or expired token")

        full_user = auth_manager.get_user(jwt_user["user_id"]) or {}
        return {
            **full_user,
            **jwt_user,
...
async def require_federation_or_admin(request: Request):
    """Accept either federation key auth OR admin user auth."""
    if _federation_secret_ok(request):
        return {"authenticated": True, "method": "federation_key"}
...
async def deregister_peer(
    ...
    """Peer-initiated deregistration. Authenticated via federation shared key."""
    if not _federation_secret_ok(request):
        raise HTTPException(status_code=401, detail="Federation authentication failed")
    await registry.deregister_node(payload.node_id)
    return {"deleted": True, "node_id": payload.node_id}
...
def _calling_peer_id(request: Request, auth: Any) -> Optional[str]:
    """Node id of the calling federation peer, or None for local admin/user.

    `_federation_secret_ok` stashes the verified node id on request.state;
    require_federation_or_admin returns {"method": "federation_key"} for
    peer callers and a user dict for admin sessions.
    """
    if isinstance(auth, dict) and auth.get("method") == "federation_key":
        return getattr(request.state, "federation_node_id", None) or "unknown"
    return None


async def _trust_filter_catalog(
    catalog: list, request: Request, auth: Any, registry: NodeRegistry
) -> list:
    """Apply trust-mode advertisement filtering when the caller is a peer.

    Local admins see the full catalog; federation peers see only the
    service types their federation_peers row publishes to them.
    """
    peer_id = _calling_peer_id(request, auth)
    if peer_id is None:
        return catalog
    from federation.trust import get_trust_enforcer
    enforcer = get_trust_enforcer(registry.db_pool, local_node_id=_local_node_id())
    return await enforcer.filter_catalog_for_peer(catalog, peer_id)


@router.get("/services")
async def list_services(
    request: Request,
    auth=Depends(require_federation_or_admin),
    registry: NodeRegistry = Depends(get_registry),
):
    catalog = await registry.get_service_catalog()
    catalog = await _trust_filter_catalog(catalog, request, auth, registry)
...
async def list_services_by_type(
    ...
    catalog = await registry.get_service_catalog(service_type=service_type)
    catalog = await _trust_filter_catalog(catalog, request, auth, registry)
    return {"services": catalog}
...
async def list_federation_agents(
    ...
    including which node hosts each agent. Peers only see agents when
    their trust row publishes the `agents` service type to them.
    """
    catalog = await registry.get_service_catalog()
    catalog = await _trust_filter_catalog(catalog, request, auth, registry)

    # Per-agent narrowing for federation peers with resource-scoped grants
    # (publish ["agents/sql-analyst"] → only that agent is listed).
    visible_agents = None
    peer_id = _calling_peer_id(request, auth)
    if peer_id is not None:
        from federation.trust import get_trust_enforcer
        enforcer = get_trust_enforcer(registry.db_pool, local_node_id=_local_node_id())
        visible_agents = await enforcer.visible_resources(peer_id, "agents")
    ...
    for service in catalog:
        if service.get("service_type") == "agents":
            ...
            for agent_id in service.get("models", []):
                if visible_agents is not None and agent_id not in visible_agents:
                    continue
                agents.append({
                    "agent_id": agent_id,
                    "node_id": node_id,
...
class FederatedAgentInvokePayload(BaseModel):
    task: str
    context: Optional[str] = None
    org_id: Optional[str] = None  # consuming org; X-Federation-Org-Id wins


@router.post("/agents/{agent_id}/invoke")
@rate_limit("federation_route")
async def federated_agent_invoke(
    agent_id: str,
    payload: FederatedAgentInvokePayload,
    request: Request,
    registry: NodeRegistry = Depends(get_registry),
):
    """Invoke a local Brigade agent on behalf of a federated consumer.

    Layer-2 contract: the peer authenticates with federation credentials and
    carries the consuming org's identity; trust modes gate per agent
    (consume ["agents"] = all, ["agents/<id>"] = that agent); a completed
    invocation fires exactly one `agent_invocation` Lago event keyed to the
    org (outcome pricing — the agent's LLM tokens are publisher COGS).
    """
    from federation.agent_bridge import ORG_HEADER, serve_federated_agent_invoke
    from federation.gateway_bridge import FederatedInferenceError

    if not _federation_secret_ok(request):
        raise HTTPException(status_code=401, detail="Federation authentication failed")
    peer_id = getattr(request.state, "federation_node_id", None) or "unknown"
    org_id = request.headers.get(ORG_HEADER) or payload.org_id
    ...
    try:
        return await serve_federated_agent_invoke(
            ...
            db_pool=registry.db_pool,
            local_node_id=_local_node_id(),
        )
    except FederatedInferenceError as exc:
...
async def route_inference(
    ...
    # Trust-mode gate: when the caller authenticated as a federation peer,
    # its federation_peers row must allow CALLING this service type on us
    # (full/publisher = all, scoped = consume[] ACL, consumer/isolated = none).
    # _federation_secret_ok() stashes the verified peer node id on request.state.
    if _federation_secret_ok(request):
        peer_id = getattr(request.state, "federation_node_id", None) or "unknown"
        from federation.trust import get_trust_enforcer
        enforcer = get_trust_enforcer(registry.db_pool, local_node_id=_local_node_id())
        allowed, reason = await enforcer.peer_may_call(peer_id, payload.service_type)
        audit = get_routing_audit_log(registry.db_pool)
        await audit.log_decision(
            request_id=f"trust-{peer_id}"[:36], service_type=payload.service_type,
            model=payload.model, user_id=payload.user_id, user_tier=payload.user_tier,
            candidates_found=0, candidates_after_constraints=0,
            selected_target="inbound", selected_node_id=peer_id,
            selected_reason=f"trust: {reason}",
            outcome="trust_allowed" if allowed else "trust_denied",
        )
        if not allowed:
            raise HTTPException(
                403, f"Trust mode denies peer '{peer_id}' calling "
                     f"'{payload.service_type}': {reason}"
            )
...
class FederatedInferencePayload(BaseModel):
    service_type: Literal["llm", "tts", "stt", "embeddings", "image_gen", "reranker"]
    model: Optional[str] = None
    org_id: Optional[str] = None  # consuming org; X-Federation-Org-Id header wins
    payload: Dict[str, Any] = Field(default_factory=dict)  # OpenAI-style body


@router.post("/inference")
@rate_limit("federation_route")
async def federated_inference(
    payload: FederatedInferencePayload,
    request: Request,
    registry: NodeRegistry = Depends(get_registry),
):
    """Serve an inbound federated inference call under the CONSUMING org's
    per-org gateway key.

    The calling peer authenticates with federation credentials and carries
    the consuming org's identity (X-Federation-Org-Id). We provision/reuse
    that org's virtual key on OUR gateway (user_id == org_id) and forward
    the payload under it, so the gateway fires exactly one Lago
    `ai_api_call` event keyed to the org with the configured markup. This
    is how the per-org billing contract survives the federation hop.
    """
    ...
    from federation.gateway_bridge import (
        ...
        serve_federated_inference_stream,
    )
    ...
    if payload.model and "model" not in body:
        body["model"] = payload.model

    # Streaming: relay the gateway's SSE token-by-token. Prep (trust/key/url)
    # runs eagerly and maps failures to a real HTTP status BEFORE any bytes are
    # committed; only gateway-level errors degrade to an in-band SSE error.
    if body.get("stream"):
        try:
...
@router.get("/topology")
async def topology(
    _auth=Depends(require_federation_or_admin),
    registry: NodeRegistry = Depends(get_registry),
):
