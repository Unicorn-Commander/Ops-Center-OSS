...
from auth_dependencies import require_authenticated_user  # per-user gate for account-facing endpoints under the m2m-allowlisted /api/v1/llm prefix
import stripe
...
class ChatMessage(BaseModel):
    ...
    content: Optional[str] = Field(None, description="Message content (null for assistant tool_calls)")
    ...
    name: Optional[str] = Field(None, description="Function name (for tool role messages)")
...
async def get_user_id(
    ...
    if token.startswith('sk-'):
        ...
        service_org_ids = {
            ...
            'majiks-service': 'a1b2c3d4-e5f6-7890-abcd-ef1234567890'        # Majiks Studio service org
        }
...
async def chat_completions(
    ...
    try:
        ...
        estimated_tokens = sum(len((msg.content or "").split()) * 1.5 for msg in request.messages)
        ...
        # Serialize messages preserving tool_calls, tool_call_id, name fields
        serialized_messages = []
        ...
        # Authoritative local/cloud signal for billing. Set True only when the
        # resolved provider record is a local backend (see ~line 1141). Default
        # False so the federation branch and any early return bill as cloud-safe
        # (federation is gateway_metered/zeroed regardless). NEVER infer local
        # from the model string — real local names lack ollama/vllm prefixes.
        is_local_provider = False
        # Local-pricing state (set by the gate after the provider is resolved):
        local_rate_per_1k = None      # metered-mode per-1k local rate (None = free)
        record_local_usage = False    # whether to add served tokens to the monthly local quota

        # Federated local models: if a trusted federation peer publishes the
        # requested model (e.g. bigboy's GPUs), serve it through the
        # federation runtime. The PUBLISHER's gateway meters the call under
        # this org's per-org key (the single Lago ai_api_call event), so no
        # local credits are charged here (gateway_metered).
        if request.model and not using_byok:
            fed_node = None
            try:
                from federation_llm_bridge import (
                    federated_node_for_model,
                    serve_llm_via_federation,
                )
                fed_node = (
                    await federated_node_for_model(request.model)
                    or await federated_node_for_model(proxy_request.get("model"))
                )
            except Exception as fed_exc:
                logger.debug(f"Federated catalog unavailable: {fed_exc}")
            if fed_node:
                org_id_fed = None
                if str(user_id).startswith("org_"):
                    org_id_fed = user_id
                else:
                    try:
                        org_id_fed = await get_org_credit_integration().get_user_org_id(user_id)
                    except Exception:
                        org_id_fed = None
                if not org_id_fed:
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            f"Model '{request.model}' is served via federation "
                            f"and requires an organization context"
                        ),
                    )
                # Streaming: relay the publisher's SSE token-by-token (the
                # publisher's gateway meters the call; this is a text/event-stream
                # response so the credit middleware charges nothing locally).
                if request.stream:
                    from federation_llm_bridge import stream_llm_via_federation
                    return StreamingResponse(
                        stream_llm_via_federation(
                            model=request.model,
                            node_id=fed_node,
                ...
                try:
                    fed_response, fed_meta = await serve_llm_via_federation(
                        model=request.model,
                        node_id=fed_node,
                        payload=proxy_request,
                        org_id=org_id_fed,
                    ...
                    logger.error(
                        f"Federated serving failed for {request.model} via {fed_node}: {fed_exc}"
                    )
                    raise HTTPException(
                        status_code=502,
                        detail=f"Federated model '{request.model}' is temporarily unavailable",
                    )
                publisher_status = fed_meta.get("publisher_status") or 200
                if publisher_status >= 400:
                    raise HTTPException(
                        status_code=publisher_status,
                        detail=f"Federated gateway error for '{request.model}'",
                    )
                if not isinstance(fed_response, dict):
                    fed_response = {"response": fed_response}
                # gateway_metered: the publisher billed this call — the credit
                # middleware honors this flag and charges nothing locally.
                fed_response.setdefault("_metadata", {})
                fed_response["_metadata"].update({
                    "federated_node": fed_node,
        ...
        if using_byok:
            ...
            async with credit_system.db_pool.acquire() as conn:
                ...
                # --- Per-tier local-pricing gate (inert unless LOCAL_PRICING_ENABLED) ---
                # Runs only for LOCAL providers. resolve_local_pricing decides:
                #   metered  -> keep model, price local at local_rate_per_1k;
                #   overflow -> re-resolve provider for a PAID model (bills as cloud);
                #   deny     -> 429; otherwise serve free (today's behavior).
                # Fail-open: any error serves the request unchanged.
                if is_local_provider:
                    try:
                        from local_pricing import resolve_local_pricing, OVERFLOW, DENY
                        _dec = await resolve_local_pricing(
                            org_id, model_name, estimated_tokens,
                            tier_code=user_tier, is_local=True,
                        )
                        ...
                        if _dec["decision"] == OVERFLOW and _dec.get("model"):
                            overflow_model = _dec["model"]
                            logger.info(f"[local_pricing] org={org_id} local quota exceeded -> overflow to '{overflow_model}'")
                            ...
                            if ov:
                                ...
                                request.model = overflow_model
                                proxy_request["model"] = normalize_model_id(overflow_model)
                                provider_db_config = provider['config'] or {}
                                if isinstance(provider_db_config, str):
                                    import json as _json
                                    provider_db_config = _json.loads(provider_db_config)
                                base_url = provider.get('api_base_url') or provider_db_config.get('base_url', 'https://openrouter.ai/api/v1')
                                provider_name = provider['name']
                                # Re-derive locality for the overflow provider (almost always cloud).
                                is_local_provider = provider['type'] in ('openai_compatible', 'local') and (
                                    provider.get('api_base_url', '').startswith('http://') or
                                    'localhost' in provider.get('api_base_url', '') or
                                    'unicorn-' in provider.get('api_base_url', '')
                                ...
                                logger.warning(f"[local_pricing] overflow model '{overflow_model}' not found; serving local")
                        elif _dec.get("meter") and _dec.get("local_rate_per_1k") is not None:
                            local_rate_per_1k = _dec["local_rate_per_1k"]
                        # Record served tokens against the monthly local quota only when
                        # the served provider is (still) local.
                        record_local_usage = bool(org_id) and is_local_provider
                    except HTTPException:
                        raise
                    except Exception as _gate_err:
                        logger.warning(f"[local_pricing] gate error (fail-open, serving unchanged): {_gate_err}")
                ...
                try:
                    ...
                    if not using_byok and (user_tier != 'free' or total_tokens > 0):
                        ...
                        actual_cost = await credit_system.calculate_cost(
                            ...
                            local_rate_per_1k=local_rate_per_1k
                        )
                        ...
                        # Count local tokens toward the org's monthly quota (mode-2 free_quota_overflow).
                        if record_local_usage:
                            try:
                                from local_quota import add_local_tokens
                                await add_local_tokens(org_id, tokens_for_billing)
                            except Exception as _q_err:
                                logger.debug(f"[local_pricing] quota record skipped: {_q_err}")
        ...
        if record_local_usage:
            try:
                from local_quota import add_local_tokens
                await add_local_tokens(org_id, tokens_used)
            except Exception as _q_err:
                logger.debug(f"[local_pricing] quota record skipped: {_q_err}")
        ...
        response_data['_metadata'] = {
            ...
            # This handler is the authoritative debit point (org/individual
            # deduction + Lago metering already happened above). The credit
            # middleware MUST NOT debit again — it keys off this flag.
...
async def generate_image(
    ...
    try:
        ...
        response_data['_metadata'] = {
            ...
            'size': request.size,
            'quality': request.quality,
            # Authoritative debit point — credit middleware must not re-debit.
            'credits_settled': True
        }
...
async def list_models(
    ...
    try:
        ...
        # Federated local models: published by a trusted peer (e.g. bigboy's
        # GPUs) and served through this node's LLM API. Visible to every tier
        # since the federated branch resolves them regardless of tier.
        try:
            from federation_llm_bridge import federated_models
            seen = {m['id'] for m in available_models}
            for model_id, node_id in (await federated_models()).items():
                if model_id and model_id not in seen:
                    available_models.append({
                        'id': model_id, 'object': 'model',
                        'owned_by': 'federation', 'tier': tier,
                        'federated_node': node_id,
                        'description': f'Local model served via federation ({node_id})',
                    })
                    seen.add(model_id)
        except Exception as fed_err:
            logger.debug(f"Federated model listing skipped: {fed_err}")
    ...
    try:
        ...
        try:
            async with credit_system.db_pool.acquire() as conn:
                ...
                if tier_result:
                    tier_markup = float(tier_result['llm_markup_percentage'] or 0)
        ...
        litellm_key = os.getenv("LITELLM_MASTER_KEY", "sk-REDACTED_LITELLM_MASTER_KEY")
        ...
        # Federated local models: published by a trusted peer (e.g. bigboy's
        # GPUs) and served through this node's LLM API. They are platform
        # models (metered on the org's per-org gateway key), grouped on their
        # own so the UI can show "local / sovereign" provenance.
        try:
            from federation_llm_bridge import federated_models
            listed_ids = {
                m['id']
                for grp in (byok_models + platform_models)
                for m in grp['models']
            }
            fed_by_node = {}
            for model_id, node_id in (await federated_models()).items():
                if model_id and model_id not in listed_ids:
                    fed_by_node.setdefault(node_id, []).append({
                        'id': model_id, 'object': 'model', 'name': model_id,
                        'display_name': model_id,
                        'federated_node': node_id,
                    })
            for node_id, models_list in fed_by_node.items():
                platform_models.append({
                    'provider': f'Federation · {node_id}',
                    'provider_type': 'federation',
                    'models': models_list,
                    'count': len(models_list),
                    'tier_markup': tier_markup,
                    'note': f'Local model served via federation from {node_id}',
                    'source': 'federation',
                    'federated_node': node_id,
                })
        except Exception as fed_err:
            logger.debug(f"Federated model categorization skipped: {fed_err}")
        ...
        # Graceful: the upstream LiteLLM proxy / OpenRouter may be unreachable on this node
        # (DNS / not configured). Return an empty-but-valid categorized payload so the
        # models screen renders "none available" instead of a 500.
        logger.warning(f"Model categorization unavailable, returning empty: {e}")
        return {"categories": {}, "providers": {}, "total": 0, "note": "Model catalog unavailable on this node"}
...
async def list_supported_providers(_user: Dict[str, Any] = Depends(require_authenticated_user)):
    """
