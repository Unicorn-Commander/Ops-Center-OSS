        ...
        async with pool.acquire() as conn:
            ...
            active_subs = await conn.fetch("""
                SELECT
                    ...
                    SUM(CASE WHEN subscription_plan IN ('managed', 'professional') THEN 1 ELSE 0 END) as platform_count,
                    SUM(CASE WHEN subscription_plan = 'byok' THEN 1 ELSE 0 END) as byok_count,
                    SUM(CASE WHEN subscription_plan IN ('enterprise', 'vip_founder') THEN 1 ELSE 0 END) as hybrid_count
            ...
            mrr_data = await conn.fetch("""
                SELECT
                    SUM(COALESCE(monthly_price, 0)) as mrr
                FROM organization_subscriptions
                WHERE status = 'active'
            """)
        ...
        async with pool.acquire() as conn:
            ...
            revenue_data = await conn.fetch(f"""
                SELECT
                    ...
                    SUM(CASE
                        WHEN subscription_plan = 'platform' THEN 50.00
                        WHEN subscription_plan = 'byok' THEN 30.00
                        WHEN subscription_plan = 'hybrid' THEN 99.00
                        ELSE 0
                    END) as revenue
                FROM organization_subscriptions
            ...
            if metric == "spend":
                ...
                query = """
                    SELECT
                        ...
                        s.subscription_plan,
                        CASE
                            WHEN s.subscription_plan = 'platform' THEN 50.00
                            WHEN s.subscription_plan = 'byok' THEN 30.00
                            WHEN s.subscription_plan = 'hybrid' THEN 99.00
                            ELSE 0
                        END as monthly_spend,
                        s.created_at as subscription_start
                    FROM organizations o
                    JOIN organization_subscriptions s ON o.id = s.org_id
                ...
                query = """
                    ...
                    FROM organizations o
                    JOIN organization_credit_pools cp ON o.id = cp.org_id
    ...
    try:
        ...
        async with pool.acquire() as conn:
            report_data = await conn.fetch("""
                ...
                FROM organizations o
                JOIN organization_subscriptions s ON o.id = s.org_id
                LEFT JOIN organization_credit_pools cp ON o.id = cp.org_id
            ...
            if format == "csv":
                ...
                writer = csv.DictWriter(output, fieldnames=[
                    'org_id', 'organization_name', 'subscription_plan', 'status',
