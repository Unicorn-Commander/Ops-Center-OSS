...
class CreditSystem:
    ...
    async def debit_credits(
        ...
        try:
            ...
            # ------------------------------------------------------------------
            # APPEND-ONLY HOOK (T7): low-credit email notification.
            # Fires at most once, when this debit made the balance CROSS below
            # 10% of the user's allocation (dispatcher re-checks prefs + applies
            # a 1/hour/user rate limit). Fire-and-forget: must NEVER delay or
            # break the debit path.
            # ------------------------------------------------------------------
            try:
                if user_id and not user_id.startswith('org_'):
                    from notification_dispatcher import maybe_notify_low_credit
                    asyncio.create_task(
                        maybe_notify_low_credit(
                            user_id=user_id,
                ...
                logger.warning(f"Low-credit notification hook failed (non-blocking): {notify_exc}")
    ...
    async def calculate_cost(
        ...
        is_local: bool = False,
        local_rate_per_1k: Optional[float] = None,
    ) -> float:
        """
        ...
        Args:
            ...
            is_local: True when the request was served by a LOCAL provider. This is
                the AUTHORITATIVE local/cloud signal — it must come from the resolved
                provider record (is_local_provider), NEVER inferred from the model
                string. Real local model names (e.g. ``qwen3.6-35b``, ``Qwen3-30B-Q4_K_M``)
                do NOT carry an ``ollama/``/``vllm/`` prefix, so ``_extract_provider``
                would mis-classify them as cloud ($0.01/1k). See Risk B.
            local_rate_per_1k: Per-1k local rate for the *metered* local-pricing mode.
                When None, local is free ($0). Ignored unless ``is_local``.
        ...
        try:
            ...
            if is_local:
                # Authoritative: local is free by default, or the metered local rate.
                base_cost_per_1k = (
                    local_rate_per_1k if local_rate_per_1k is not None else PRICING["local"]
                )
            ...
            # Return safe default — but a LOCAL request must never fall back to a
            # cloud rate (Risk B): local stays free (or its metered rate).
            if is_local:
                return round((tokens_used / 1000) * (local_rate_per_1k or 0.0), 6)
    ...
    async def get_user_tier(self, user_id: str) -> str:
        ...
        try:
            # Check if this is a service organization ID
            if user_id and user_id.startswith('org_'):
                # Strip org_ prefix to get UUID for DB lookup (id column is UUID type)
                org_uuid = user_id[4:]
                async with self.db_pool.acquire() as conn:
                    # plan_tier is present on all org schemas; subscription_tier may not exist
                    tier = await conn.fetchval(
                        """
                        SELECT plan_tier FROM organizations WHERE id::text = $1
                        """,
                        org_uuid
                    )
                    if tier:
