...
# NOTE: the `database` PACKAGE (database/__init__.py) shadows database.py, and its
# get_db_connection() returns a PoolAcquireContext meant for `async with`, NOT a live
# connection. Every handler here uses the pattern `conn = await get_db_connection(); ...;
# await conn.close()`, which against the package yields a PoolAcquireContext and blows up
# with "'PoolAcquireContext' object has no attribute 'close'". Wrap it so callers get a
# real pooled connection (await the context to acquire it). conn.close() on a pooled
# connection is acceptable here (admin dashboards, low frequency).
from database import get_db_connection as _pkg_get_db_connection


async def get_db_connection():
    """Return a live pooled connection (await the package's PoolAcquireContext)."""
...
async def get_user_billing_dashboard(request: Request):
    ...
    **Returns**:
    ...
    conn = await get_db_connection()
    try:
        # Get all organizations user belongs to (with credit allocation + subscription).
        # Wrap in try/except so a missing credit/subscription table or column still
        # returns the user's basic org membership with zeroed usage (graceful).
        try:
            ...
            logger.warning(
                f"Credit allocation query failed for user {user['user_id']}, "
                f"falling back to basic org info: {e}"
            )
            # Fall back to basic org membership only (no credit/subscription columns)
            orgs = await conn.fetch(
                """
                SELECT
                    o.id as org_id,
                    ...
                    NULL::numeric as allocated_credits,
                    NULL::numeric as used_credits,
                    NULL::numeric as remaining_credits
                FROM organization_members om
                JOIN organizations o ON om.org_id = o.id
                WHERE om.user_id = $1 AND o.status = 'active'
                ORDER BY o.name
        ...
        # Wrap so any DB error still returns zeroed usage (graceful), HTTP 200.
        try:
            ...
            logger.warning(
                f"Usage attribution query failed for user {user['user_id']}, "
                f"returning zeroed usage: {e}"
            )
            total_usage = None
        ...
        return {
            ...
            "total_usage_last_30_days": {
                "total_credits_used": (total_usage["total_credits_used"] or 0) if total_usage else 0,
                "org_count": (total_usage["org_count"] or 0) if total_usage else 0,
                "request_count": (total_usage["request_count"] or 0) if total_usage else 0
            }
        }
            ...
            "organizations": [
                {
                    ...
                    "lifetime_spent": float(org["lifetime_spent_amount"] or 0)
                }
                for org in orgs
            ],
            "revenue_metrics": {
