...
run_smoke_tests() {
    ...
    if docker exec ops-center-direct python -c "import psycopg2; conn = psycopg2.connect(host='unicorn-postgresql', user='unicorn', password='REDACTED_POSTGRES_PASSWORD', dbname='unicorn_db'); conn.close()" 2>/dev/null; then
