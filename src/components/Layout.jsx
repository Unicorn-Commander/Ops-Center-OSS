...
const GuideChatBubble = lazy(() => import('./GuideChatBubble'));
...
export default function Layout({ children }) {
  ...
  const currentTitle = currentRoute?.name || branding?.company_name || 'Ops-Center';

  // Keep the browser-tab title in sync with the current page
  useEffect(() => {
    const brand = branding?.company_name || 'Ops-Center';
    document.title = currentRoute?.name ? `${currentRoute.name} · ${brand}` : brand;
  }, [currentRoute?.name, branding?.company_name]);
  ...
  return (
    <div className={themeClasses.background}>
          ...
          <div className={`flex flex-col flex-grow pt-5 pb-4 overflow-y-auto overflow-x-hidden ${themeClasses.sidebar}`}>
            ...
            <div className={`flex flex-col items-center flex-shrink-0 mb-8 transition-all duration-300 ${
              ...
              <Link
                ...
                title={sidebarCollapsed ? `${branding?.company_name || 'Ops-Center'} - ${branding?.subtitle || 'Operations Console'}` : ""}
              >
                {/* Tenant logo from BrandingContext, with MagicUnicornLogo fallback */}
                {branding?.logo_url ? (
                  <img
                    src={branding.logo_url}
                    alt={branding.company_name || 'Ops-Center'}
                    className={`drop-shadow-xl object-contain ${sidebarCollapsed ? 'w-10 h-10' : 'w-14 h-14'}`}
                  />
                ) : (
                ...
                {!sidebarCollapsed && (
                  <div className="text-center">
                    <h1 className={`text-2xl font-bold ${themeClasses.logo} leading-tight`}>
                      {branding?.company_name || 'Ops-Center'}
                    </h1>
                    <div className={`text-lg ${currentTheme === 'unicorn' ? 'text-purple-200/80' : currentTheme === 'light' ? 'text-gray-600' : 'text-gray-400'} font-medium`}>
                      {branding?.subtitle || 'Operations Console'}
                    </div>
                  </div>
                )}
              </Link>

              {/* Powered By - tagline + federation upstream attribution (hidden when collapsed) */}
              {!sidebarCollapsed && (
                <>
            ...
            <div className="mt-4 flex flex-col flex-1">
              ...
              {/* Help Button — opens the in-app Help panel (App.jsx listens for this event).
                  Used to open http://<host>:8086, a legacy appliance docs port that is dead
                  on cloud deployments. */}
              <div className="px-2 mb-4">
                <button
                  onClick={() => {
                    window.dispatchEvent(new CustomEvent('uc:open-help'));
                  }}
      ...
      {/* The Guide - end-user assistant (read-only, no system access). Shown to
          non-admins as their floating help agent (Colonel is admin-only). */}
      {userRole !== 'admin' && (
        <Suspense fallback={null}>
          <GuideChatBubble />
        </Suspense>
